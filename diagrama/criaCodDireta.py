...
def criar_cases():
	...
	for key, value in diagrama.items():
			...
			for final in finalApareceu:
				...
				if final != 'AS' and final != 'X' and final != 'ER':
					if final not in ['AP', 'AO', 'AR']:
						...
						file.write(f"\t\t\t\t\t\treturn Token(tipo ,id[:-1].strip(),linhaGlobal,coluna)\n")
						file.write(f"\t\t\t\t\telse:\n")
						file.write(f"\t\t\t\t\t\ttabelaSimbolos[id.strip()] = Atributo(tipo,id.strip())\n")
						file.write(f"\t\t\t\t\t\treturn Token(tipo,id.strip(),linhaGlobal,coluna)\n")
						file.write(f"\t\t\t\t\ttabelaSimbolos[id[:-1]] = Atributo(tipo,id[:-1])\n")
						file.write(f"\t\t\t\t\treturn Token(tipo,id[:-1],linhaGlobal,coluna)\n")
						...
						file.write(f"\t\t\t\treturn Token('op_rela',id.strip(),linhaGlobal,coluna)\n")
				elif final == 'X':
					file.write(f"\t\t\t\tlerProx = True\n")
				elif final == 'ER':
		...
		if key_[0] == 'AN':
			...
			file.write(f"\t\t\t\t\t\treturn Token(tipo,id,linhaGlobal,coluna)\n")
			file.write(f"\t\t\t\t\telse:\n")
			file.write(f"\t\t\t\t\t\ttabelaSimbolos[id.strip()] = Atributo(tipo,id.strip())\n")
			file.write(f"\t\t\t\t\t\treturn Token(tipo,id.strip(),linhaGlobal,coluna)\n")
