...
def criar_cases():
	...
	for key, value in diagrama.items():
		...
		if key_[0] != estadoAnterior:
			for final in finalApareceu:
				...
				if final != 'AS' and final != 'X' and final != 'ER':
					...
					file.write(f"\t\t\t\t\t\ttabelaSimbolos[id[:-1].strip()] = Atributo(id[:-1].strip(),'')\n")
					...
					file.write(f"\t\t\t\t\ttabelaSimbolos[id[:-1]] = Atributo(id[:-1],'')\n")
					file.write(f"\t\t\t\t\treturn Token(id[:-1],'ATRIBUTO',linhaGlobal,coluna)\n")
					...
					file.write(f"\t\t\t\treturn Token('Erro','ATRIBUTO',linhaGlobal,colunaGlobal)\n")
				else:
					file.write(f"\t\t\t\tlerProx = False\n")
		...
		if key_[0] == 'AN':
			...
			file.write(f"\t\t\t\t\t\treturn Token(id[:-1],'ATRIBUTO',linhaGlobal,coluna)\n")
			file.write(f"\t\t\t\t\telif c in ['\\n', '\\t', ' ',';',')','+']:\n")
			file.write(f"\t\t\t\t\t\ttabelaSimbolos[id] = Atributo(id,'')\n")
			file.write(f"\t\t\t\t\t\treturn Token(id,'ATRIBUTO',linhaGlobal,coluna)\n")
			file.write(f"\t\t\t\t\telse:\n")
			file.write(f"\t\t\t\t\t\ttabelaSimbolos[id.strip()] = Atributo(id.strip(),'')\n")
			file.write(f"\t\t\t\t\t\treturn Token(id.strip(),'ATRIBUTO',linhaGlobal,coluna)\n")
		...
		match state:
			...
			case 'L':
				...
				if c not in ['\n', '\t', ' ','+','-','/',',','*',':',';','(',')','=']:
					...
					if id != c:
						f.seek(f.tell()-1)
						tabelaSimbolos[id[:-1].strip()] = Atributo(id[:-1].strip(),'op_arit')
						return Token(id[:-1].strip(),'ATRIBUTO',linhaGlobal,coluna)
					else:
						tabelaSimbolos[id.strip()] = Atributo(id.strip(),'op_arit')
						return Token(id.strip(),'ATRIBUTO',linhaGlobal,coluna)
					tabelaSimbolos[id[:-1]] = Atributo(id[:-1],'op_arit')
			...
			case 'R':
				...
				if c not in ['\n', '\t', ' ','+','-','/',',','*',':',';','(',')','=']:
					...
					if id != c:
						f.seek(f.tell()-1)
						tabelaSimbolos[id[:-1].strip()] = Atributo(id[:-1].strip(),'op_rela')
						return Token(id[:-1].strip(),'ATRIBUTO',linhaGlobal,coluna)
					else:
						tabelaSimbolos[id.strip()] = Atributo(id.strip(),'op_rela')
						return Token(id.strip(),'ATRIBUTO',linhaGlobal,coluna)
					tabelaSimbolos[id[:-1]] = Atributo(id[:-1],'op_rela')
					return Token(id[:-1],'ATRIBUTO',linhaGlobal,coluna)
				elif c in [' ',';',')','+']:
					print(id)
					print(c)
			...
			case 'AT':
				...
				if c not in ['\n', '\t', ' ','+','-','/',',','*',':',';','(',')','=']:
					...
					if id != c:
						f.seek(f.tell()-1)
						tabelaSimbolos[id[:-1].strip()] = Atributo(id[:-1].strip(),id[:-1].strip())
						return Token(id[:-1].strip(),'ATRIBUTO',linhaGlobal,coluna)
					else:
						tabelaSimbolos[id.strip()] = Atributo(id.strip(),id.strip())
						return Token(id.strip(),'ATRIBUTO',linhaGlobal,coluna)
					tabelaSimbolos[id[:-1]] = Atributo(id[:-1],'')
					return Token(id[:-1],'ATRIBUTO',linhaGlobal,coluna)
				elif c in [' ',';',')','+']:
			...
			case 'CA':
				...
				if c not in ['\n', '\t', ' ','+','-','/',',','*',':',';','(',')','=']:
					...
					if id != c:
						...
						tabelaSimbolos[id.strip()] = Atributo(id.strip(),id.strip())
						return Token(id.strip(),'ATRIBUTO',linhaGlobal,coluna)
					tabelaSimbolos[id[:-1]] = Atributo(id[:-1],id[:-1].strip())
					return Token(id[:-1],'ATRIBUTO',linhaGlobal,coluna)
				elif c in [' ',';',')','+']:
					print(id)
					print(c)
