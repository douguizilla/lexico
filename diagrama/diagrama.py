...
def criar_cases():
	...
	for key, value in diagrama.items():
			...
			for final in finalApareceu:
				...
				if final != 'AS' and final != 'X' and final != 'ER':
					if final not in ['AP', 'AO', 'AR']:
						...
						file.write(f"\t\t\t\t\t\treturn Token(id[:-1].strip(),id[:-1].strip(),linhaGlobal,coluna)\n")
						file.write(f"\t\t\t\t\telse:\n")
						...
						file.write(f"\t\t\t\t\treturn Token(id[:-1],id[:-1],linhaGlobal,coluna)\n")
						...
						file.write(f"\t\t\t\t\telse:\n")
						file.write(f"\t\t\t\t\t\ttabelaSimbolos[id.strip()] = Atributo(tipo,id.strip())\n")
						file.write(f"\t\t\t\t\t\treturn Token(id.strip(),id.strip(),linhaGlobal,coluna)\n")
						file.write(f"\t\t\t\telse:\n")
						file.write(f"\t\t\t\t\tprint(id)\n")
						file.write(f"\t\t\t\t\tprint(c)\n")
						file.write(f"\t\t\t\t\tif id != c:\n")
					...
					file.write(f"\t\t\t\treturn Token('Erro','',linhaGlobal,colunaGlobal)\n")
				else:
					file.write(f"\t\t\t\tlerProx = False\n")
		...
		if key_[0] == 'AN':
			...
			file.write(f"\t\t\t\t\t\tf.seek(f.tell()-1)\n")
			file.write(f"\t\t\t\t\t\ttabelaSimbolos[id[:-1].strip()] = Atributo(tipo,id[:-1].strip())\n")
			file.write(f"\t\t\t\t\t\treturn Token(id[:-1],id[:-1].strip(),linhaGlobal,coluna)\n")
			file.write(f"\t\t\t\t\telif c in ['\\n', '\\t', ' ',';',')','+']:\n")
			file.write(f"\t\t\t\t\t\ttabelaSimbolos[id] = Atributo(tipo,id)\n")
			file.write(f"\t\t\t\t\t\treturn Token(id,id,linhaGlobal,coluna)\n")
			file.write(f"\t\t\t\t\telse:\n")
			file.write(f"\t\t\t\t\t\ttabelaSimbolos[id.strip()] = Atributo(tipo,id.strip())\n")
			file.write(f"\t\t\t\t\t\treturn Token(id.strip(),id.strip(),linhaGlobal,coluna)\n")
		...
		match state:
			...
			case 'AN':
				...
				if c.lower() == '-':
					...
					state = 'A'
					lerProx = False
					tipo = 'op_rela' if id in ['<','>','<>','<=','>='] else ('op_arit' if id in ['+','-','*','/','^'] else ( id if id in ['se','senao','entao','ate','enquanto', 'programa', 'inicio', 'fim','faca','repita'] else ''))
					if c not in ['\n', '\t', ' ','+','-','/',',','*',':',';','(',')','=']:
						f.seek(f.tell()-1)
						tabelaSimbolos[id[:-1].strip()] = Atributo(tipo,id[:-1].strip())
